import { DEFAULT_OLLAMA_CHAT_MODEL, LESSON29_ENDPOINT, requestLesson29Answer } from "./lesson-29-api.js";
...
if (requiredElements.some((element) => !element)) {
  throw new Error("Lesson 29 page is missing required DOM elements.");
}
...
function initPage() {
  endpointLabel.textContent = LESSON29_ENDPOINT;
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    const payload = await requestLesson29Answer(prompt);
    responseOutput.textContent = payload.answer || "Ollama вернул пустой ответ.";
    responseOutput.scrollTop = 0;
    endpointLabel.textContent = payload.endpoint || LESSON29_ENDPOINT;
