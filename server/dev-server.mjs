...
import { runLesson29Chat } from "./lesson29-chat.mjs";
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "POST" && requestUrl.pathname === "/api/lesson25/chat") {
    try {
      const rawBody = await readRequestBody(request);
      const payload = JSON.parse(rawBody || "{}");
      const prompt = typeof payload.prompt === "string" ? payload.prompt.trim() : "";
      ...
      sendJson(response, 200, {
        ...result,
        mode: "rag-with-history"
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unexpected lesson 25 proxy error.";
      sendJson(response, 500, { error: message });
    }
    return;
  }

  if (request.method === "POST" && requestUrl.pathname === "/api/week5/ollama-chat") {
    try {
      const rawBody = await readRequestBody(request);
      const payload = JSON.parse(rawBody || "{}");
      const prompt = typeof payload.prompt === "string" ? payload.prompt.trim() : "";
  ...
  if (request.method === "POST" && requestUrl.pathname === "/api/lesson29/chat") {
    try {
      const rawBody = await readRequestBody(request);
      const payload = JSON.parse(rawBody || "{}");
      const prompt = typeof payload.prompt === "string" ? payload.prompt.trim() : "";
      ...
      const result = await runLesson29Chat({
        prompt,
        env
      });
      ...
      const message = error instanceof Error ? error.message : "Unexpected lesson 29 proxy error.";
      sendJson(response, 500, { error: message });
    }
    return;
  }
