...
4. Если проверяете урок 19, в отдельном терминале запустите MCP-сервер сохранения ответа:

```bash
npm run lesson19:mcp
```
...
- `http://localhost:4173/lessons/lesson-19/`

Урок 19 повторяет дизайн урока 17 и добавляет сохранение ответа LLM в файл через отдельный локальный MCP-сервер.

Что делает страница:

- отправляет обычный запрос к LLM через `POST /api/lesson19/chat`;
- позволяет указать абсолютный путь в поле `Папка для сохранения`;
- хранит этот путь в `localStorage`;
- после получения ответа активирует кнопку `Сохранить`;
- по кнопке вызывает backend endpoint `POST /api/lesson19/save-response`, который уже обращается к локальному MCP-серверу сохранения.

Отдельный MCP-сервер урока 19:

- script: `npm run lesson19:mcp`
- endpoint: `http://127.0.0.1:4175/mcp`
- health: `http://127.0.0.1:4175/health`

Что делает MCP tool:

- принимает абсолютный путь к папке;
- создаёт папку, если её ещё нет;
- сохраняет каждый ответ в новый файл;
- имя файла: `llm-response-YYYY-MM-DD-HH-mm-ss.txt`

Формат содержимого файла:

```text
Timestamp: 2026-06-26T12:00:00.000Z
Folder: C:\Users\Alex\Documents\LLM
...
Если путь не указан или не является абсолютным, сохранение завершается понятной ошибкой без записи файла.

### Урок 20

URL:

- `http://localhost:4173/lessons/lesson-20/`

Урок 20 использует тот же визуальный стиль, что и lesson 19, но выполняет весь flow автоматически после одного нажатия `Отправить`.

Автоматический сценарий:

1. `MCP покемонов`
2. `MCP сохранения в файл`

Что делает backend:

- сначала вызывает Pokémon MCP через тот же flow, что и lesson 17;
- если первый шаг успешен, автоматически передаёт исходный prompt, итоговый answer и `folderPath` в MCP сохранения;
- если первый шаг падает, второй шаг не запускается;
- если второй шаг падает, ответ первого шага остаётся видимым.

Backend endpoint урока 20:

- `POST /api/lesson20/auto-flow`

UI урока 20 показывает:

- статус общего flow;
- статус шага `MCP покемонов`;
- статус шага `MCP сохранения в файл`;
- итоговый ответ;
- путь к сохранённому файлу, если второй шаг завершился успешно;
- объединённый список реально вызванных MCP tools.
